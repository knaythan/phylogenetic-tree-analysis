...
def evaluate_algorithm(name, builder, *args, reference_tree=None, show_tree=False, image_dir=None):
    ...
    try:
        ...
        print(f"Error while building tree with {name}: {e}")
        tracemalloc.stop()
        return None
    end_time = time.time()
...
def process_fasta_file(fasta_path, show_trees=False):
    ...
    try:
        ...
        print(f"Skipping {base_filename}: alignment or distance matrix failed ({e})")
        return
    ...
    print(f"Saved output summary to {output_file}")
